    ...
    <script>
        ...
        (function() {
            ...
            function applyGeoRedirect(cc) {
                ...
                // Direct product page link on regional Amazon storefront (e.g. amazon.in/dp/ASIN)
                const directLocalUrl = `https://www.${target.domain}/dp/${asin}`;
                
                if (buyBtn) buyBtn.href = directLocalUrl;
                if (buyBtnText) buyBtnText.innerText = `BUY ON ${target.label}`;

                // Verify if product is listed in region or requires search fallback
                fetch(directLocalUrl, { mode: 'no-cors' }).then(() => {
                    // Direct product page available!
                }).catch(() => {
                    // Item unlisted in region -> fallback to search & show notice
                    ...
                    if (geoBox) {
                        ...
                        if (titleEl) titleEl.innerText = `Product Not Directly Listed on ${target.domain}`;
                        if (descEl) descEl.innerText = `This specific US model code is not directly listed in your country. We've automatically linked equivalent local options on ${target.domain}.`;
