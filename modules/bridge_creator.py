    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-share-pinterest:hover {
            ...
            box-shadow: 0 10px 28px rgba(230, 0, 35, 0.6);
        }
...
<body>
    ...
    <div class="container">
        ...
        <!-- 1-Click Social Share & Save Bar -->
        <div class="share-container">
            <a id="pinterestShareBtn" href="#" target="_blank" rel="noopener noreferrer" class="btn-share-pinterest">
                <span>📌 Save to Pinterest</span>
            </a>
            <button onclick="copyPageLink(this)" class="btn-copy-link">
                <span>🔗 Copy Link</span>
            </button>
        </div>

        <!-- Editorial Product Overview -->
        <div style="margin-top: 28px; padding-top: 24px; border-top: 1px solid rgba(255,255,255,0.1); text-align: left;">
            <h3 style="font-family: 'Playfair Display', serif; font-size: 18px; color: var(--accent-gold); margin-bottom: 12px;">
        ...
        <footer style="margin-top: 30px; padding-top: 20px; border-top: 1px dashed rgba(255,255,255,0.12); text-align: center; font-size: 12px; color: #64748b;">
            <p style="font-size: 12px; color: var(--accent-gold); font-weight: 700; margin-bottom: 8px;">Company: Cozy Room Finds | App: Cozy Room Decor Publisher Pro</p>
            <div style="background: rgba(255, 183, 3, 0.15); border: 1.5px solid #ffb703; border-radius: 50px; padding: 6px 20px; display: inline-block; margin: 6px 0 12px; box-shadow: 0 4px 15px rgba(255, 183, 3, 0.25);">
                <span style="color: #ffffff; font-weight: 700; font-size: 12px;">📧 Developer Contact Email: </span>
                <a href="mailto:aditya.s.nalawade742@gmail.com" style="color: #ffb703; font-weight: 800; text-decoration: underline; font-size: 13px;">aditya.s.nalawade742@gmail.com</a>
            </div>
            <div style="margin-bottom: 8px;">
                <a href="https://adityasnalawade742-design.github.io/index.html" style="color: var(--accent-gold); text-decoration: none; margin: 0 8px; font-weight: 600;">🏠 Home Showcase</a> • 
                <a href="https://adityasnalawade742-design.github.io/privacy-policy.html" style="color: var(--accent-gold); text-decoration: none; margin: 0 8px; font-weight: 600;">Privacy Policy & Developer Compliance</a> •
                <a href="https://adityasnalawade742-design.github.io/terms-of-service.html" style="color: var(--accent-gold); text-decoration: none; margin: 0 8px; font-weight: 600;">Terms of Service</a>
            </div>
    ...
    <script>
        ...
        // Auto-configure Pinterest Share Link
        (function() {
            try {
                const pageUrl = encodeURIComponent(window.location.href);
                const title = encodeURIComponent("{{ product.title }}");
                const mainImg = document.getElementById('mainImage');
                const imgSrc = mainImg ? mainImg.getAttribute('src') : "{{ hook_image_rel }}";
                const absoluteImgUrl = encodeURIComponent(new URL(imgSrc, window.location.href).href);
                const pinBtn = document.getElementById('pinterestShareBtn');
                if (pinBtn) {
                    pinBtn.href = `https://pinterest.com/pin/create/button/?url=${pageUrl}&media=${absoluteImgUrl}&description=${title}`;
                }
        ...
        (function() {
            const countryMap = {
                ...
                // AMERICAS
                "US": { domain: "amazon.com", label: "AMAZON US ($)" },
            ...
            try {
                fetch('https://open.er-api.com/v6/latest/USD')
                    .then(res => res.json())
                    .then(data => { if (data && data.rates) exchangeRates = { ...exchangeRates, ...data.rates }; if (window.currentAppliedCC) applyGeoRedirect(window.currentAppliedCC); })
                    .catch(e => {});
            } catch(e) {}
            ...
            function applyGeoRedirect(cc) {
                ...
                // B. Geo-Aware Price Display & Labeling Engine
                if (regPrice && regPrice === 'Not Available') {
                    ...
                    const isVerifiedPrice = isDirectListing;
                    const finalTagText = isVerifiedPrice ? `✨ VERIFIED DEAL • ${regPrice}` : `⚠️ UNLISTED IN REGION • Approx. ${regPrice}`;
                    const finalPriceText = isVerifiedPrice ? regPrice : `Approx. ${regPrice}`;

                    priceTags.forEach(el => {
                        if (el.classList.contains('tag')) {
                            el.innerText = finalTagText;
                            if (!isVerifiedPrice) {
                    ...
                    const isVerifiedPrice = isUsd || isDirectListing;
                    const finalTagText = isVerifiedPrice ? `✨ VERIFIED DEAL • ${displayVal}` : `⚠️ UNLISTED IN REGION • Approx. ${displayVal}`;
                    const finalPriceText = isVerifiedPrice ? displayVal : `Approx. ${displayVal}`;
                    priceTags.forEach(el => {
                        if (el.classList.contains('tag')) {
                            el.innerText = finalTagText;
                            if (!isVerifiedPrice) {
            ...
            // ⚡ Single-Resolution Network-First Auto-Detection Engine (1.5s Fallback Deadline)
            (function() {
                ...
                // Helper: Passive Timezone / Language Fallback Detection (Used ONLY on Timeout)
                function detectTimezoneOrLanguage() {
                    try {
                        ...
                        for (const [tzPart, cc] of Object.entries(tzCountryMap)) {
                            if (tz.includes(tzPart)) return cc;
                        }
                        if (lang.includes('en-in') || lang.includes('-in') || lang.includes('hi')) return 'IN';
                        if (lang.includes('en-gb') || lang.includes('-gb')) return 'GB';
                ...
                fallbackTimer = setTimeout(function() {
                    if (!isResolved) {
                        ...
                        commitResolution(fallbackCC, 'FALLBACK_TIMEOUT');
                    }
                }, 1500);

                // Step 2: Authoritative Network/IP Detection (Cloudflare trace + Cascading IP APIs)
                try {
