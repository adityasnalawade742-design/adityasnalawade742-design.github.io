...
def scrape_au_prices():
    print("🇦🇺 [6/7] SCRAPING AMAZON AUSTRALIA (Amazon.com.au)...")
    ...
    with sync_playwright() as p:
        ...
        for asin, item in registry.items():
            ...
            target_asin = item.get("regional_asins", {}).get("AU") or asin
            url = f"https://www.amazon.com.au/dp/{target_asin}"
            price_str = None
            ...
            try:
                ...
                if offscreen:
                    ...
                    if "INR" not in txt and "₹" not in txt:
                        m = re.search(r"A\$\s*(\d{1,3}(?:,\d{3})*(?:\.\d{2})?)|\$\s*(\d{1,3}(?:,\d{3})*(?:\.\d{2})?)", txt)
                        if m:
            ...
            if price_str:
                item["regional_prices"]["AU"] = price_str
                print(f"  • [{asin}] Australia Price: {price_str}")
            else:
                existing = item["regional_prices"].get("AU", "Not Available")
                item["regional_prices"]["AU"] = existing
                print(f"  • [{asin}] Australia Price (Preserved): {existing}")

        browser.close()

    registry_file.write_text(json.dumps(registry, indent=2, ensure_ascii=False), encoding="utf-8")
    print("✅ Amazon Australia Scraping Complete.\n")
