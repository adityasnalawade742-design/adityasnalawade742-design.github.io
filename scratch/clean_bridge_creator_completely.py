...
print("==================================================")
print("🧼 CLEANING modules/bridge_creator.py COMPLETELY")
print("==================================================")

bridge_creator_path = repo_dir / "modules" / "bridge_creator.py"
bc_content = bridge_creator_path.read_text(encoding="utf-8")

# Find start of exchangeRates script block down to end of script block
start_idx = bc_content.find("const exchangeRates = {")
end_idx = bc_content.find("// ⚡ Phase 0: Instant URL Test Parameter Override")

clean_js_block = """const exchangeRates = { "USD": 1.0, "EUR": 0.92, "GBP": 0.78, "CAD": 1.36, "AUD": 1.52, "INR": 83.50, "JPY": 155.0, "BRL": 5.45, "MXN": 18.20, "SGD": 1.35, "NZD": 1.64, "CHF": 0.89, "SEK": 10.50, "AED": 3.67, "SAR": 3.75, "KRW": 1380.0 };
...
if start_idx != -1 and end_idx != -1:
    bc_content = bc_content[:start_idx] + clean_js_block + bc_content[end_idx:]

bridge_creator_path.write_text(bc_content, encoding="utf-8")
print(" ✅ Cleaned modules/bridge_creator.py completely!")
...
print("\n🚀 Pushing clean landing page currency engine live to GitHub Pages...")
try:
    subprocess.run(["git", "add", "-A"], cwd=str(repo_dir), check=True)
    subprocess.run(["git", "commit", "-m", "clean modules/bridge_creator.py and rebuild 100% of landing pages"], cwd=str(repo_dir), check=True)
