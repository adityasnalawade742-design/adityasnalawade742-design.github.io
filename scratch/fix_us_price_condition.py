...
print("==================================================")
print("🌍 FIXING US PRICE CONDITION IN modules/bridge_creator.py")
print("==================================================")

bridge_creator_path = repo_dir / "modules" / "bridge_creator.py"
bc_content = bridge_creator_path.read_text(encoding="utf-8")

# Fix condition: change regKey !== 'us' to regPrice check so US prices update properly
old_cond = "} else if (regPrice && regKey !== 'us') {"
new_cond = "} else if (regPrice) {"

if old_cond in bc_content:
    bc_content = bc_content.replace(old_cond, new_cond)

bridge_creator_path.write_text(bc_content, encoding="utf-8")
print(" ✅ Fixed regPrice condition in modules/bridge_creator.py!")
...
print("\n🚀 Pushing fixed US price condition live to GitHub Pages...")
try:
    subprocess.run(["git", "add", "-A"], cwd=str(repo_dir), check=True)
    subprocess.run(["git", "commit", "-m", "fix regPrice condition so US and international prices adapt for all world countries"], cwd=str(repo_dir), check=True)
