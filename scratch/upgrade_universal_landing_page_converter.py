...
print("==================================================")
print("🌍 UPGRADING UNIVERSAL LANDING PAGE CURRENCY CONVERTER FOR ALL WORLD COUNTRIES")
print("==================================================")

bridge_creator_path = repo_dir / "modules" / "bridge_creator.py"
bc_content = bridge_creator_path.read_text(encoding="utf-8")

# Universal Exchange Rates & Symbols for Bridge Template JS
universal_js_engine = """
            ...
            function applyGeoRedirect(cc) {
                ...
                // 🏷️ Universal Dynamic Price Tag Update for EVERY Country
...
# Replace old applyGeoRedirect function block in bridge_creator.py
pattern = r"function applyGeoRedirect\(cc\) \{[\s\S]*?\}\s*\}\s*\}\s*\}"
bc_content = re.sub(r'function applyGeoRedirect\(cc\) \{[\s\S]*?\}\s*\}', universal_js_engine, bc_content)

bridge_creator_path.write_text(bc_content, encoding="utf-8")
print(" ✅ Upgraded modules/bridge_creator.py with universal world currency converter!")
...
print("\n🔨 Rebuilding 100% of landing pages with universal currency engine...")
...
print("\n🚀 Pushing universal landing page currency converter live to GitHub Pages...")
try:
    subprocess.run(["git", "add", "-A"], cwd=str(repo_dir), check=True)
    subprocess.run(["git", "commit", "-m", "upgrade landing page currency converter to adapt dynamically for every world country"], cwd=str(repo_dir), check=True)
...
print("\n🎉 UNIVERSAL WORLD CURRENCY ENGINE DEPLOYED LIVE!")
