...
print("==================================================")
print("🎯 FIXING applyGeoRedirect ORDER IN modules/bridge_creator.py")
print("==================================================")

bridge_creator_path = repo_dir / "modules" / "bridge_creator.py"
bc_content = bridge_creator_path.read_text(encoding="utf-8")

# Completely clean and elegant applyGeoRedirect implementation
clean_geo_redirect = """            const exchangeRates = { "USD": 1.0, "EUR": 0.92, "GBP": 0.78, "CAD": 1.36, "AUD": 1.52, "INR": 83.50, "JPY": 155.0, "BRL": 5.45, "MXN": 18.20, "SGD": 1.35, "NZD": 1.64, "CHF": 0.89, "SEK": 10.50, "AED": 3.67, "SAR": 3.75, "KRW": 1380.0 };
            ...
            function applyGeoRedirect(cc) {
                ...
                // 🏷️ 1. Dynamic Regional Price Tag Update (ALWAYS RUNS FOR ALL COUNTRIES)
...
# Replace in modules/bridge_creator.py
bc_content = re.sub(r'const exchangeRates = \{[\s\S]*?function applyGeoRedirect\(cc\) \{[\s\S]*?\}\s*\}', clean_geo_redirect, bc_content)

bridge_creator_path.write_text(bc_content, encoding="utf-8")
print(" ✅ Cleaned and reordered applyGeoRedirect in modules/bridge_creator.py!")
...
print("\n🚀 Pushing fixed landing page price order live to GitHub Pages...")
try:
    subprocess.run(["git", "add", "-A"], cwd=str(repo_dir), check=True)
    subprocess.run(["git", "commit", "-m", "fix applyGeoRedirect execution order so price tags update for all world countries"], cwd=str(repo_dir), check=True)
...
print("\n🎉 ALL WORLD COUNTRIES CURRENCY ENGINE DEPLOYED LIVE!")
