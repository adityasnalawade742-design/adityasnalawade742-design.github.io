...
queries = ["ceramic donut vase home decor", "aesthetic wavy mirror wall decor", "boho woven macrame wall decor"]

fresh_verified = []

print("Querying SerpAPI live Amazon catalog for fresh Home Decor items...")
for q in queries:
        ...
        for item in res:
            ...
            try:
                ...
                if 'Page Not Found' not in title and 'Robot Check' not in title and '404' not in title and len(title) > 5:
                    ...
                    print(f"[FRESH LIVE 100%] {asin} -> {title[:45]} | Price: {item.get('price')}")
                    break
            except Exception as e:
                pass

with open("fresh_serp_verified.json", "w", encoding="utf-8") as f:
    json.dump(fresh_verified, f, indent=2)
