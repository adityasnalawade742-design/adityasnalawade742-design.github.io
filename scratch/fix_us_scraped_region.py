...
print("==================================================")
print("🎯 FIXING US REGION IN EXPLICIT SCRAPED REGIONS")
print("==================================================")

bridge_creator_path = repo_dir / "modules" / "bridge_creator.py"
bc_content = bridge_creator_path.read_text(encoding="utf-8")

# Include 'us' in explicitScrapedRegions
old_arr = "const explicitScrapedRegions = ['in', 'uk', 'de', 'ca', 'jp', 'au'];"
new_arr = "const explicitScrapedRegions = ['us', 'in', 'uk', 'de', 'ca', 'jp', 'au'];"

if old_arr in bc_content:
    bc_content = bc_content.replace(old_arr, new_arr)

# Also ensure Jinja template evaluates product.get('current_price', product.get('price', '$19.99'))
old_price_call = '"{{ product.current_price }}"'
new_price_call = '"{{ product.get(\'current_price\', product.get(\'price\', \'$19.99\')) }}"'

if old_price_call in bc_content:
    bc_content = bc_content.replace(old_price_call, new_price_call)

bridge_creator_path.write_text(bc_content, encoding="utf-8")
print(" ✅ Updated modules/bridge_creator.py with US region fix!")
...
print("\n🚀 Pushing US explicit region fix live to GitHub Pages...")
try:
    subprocess.run(["git", "add", "-A"], cwd=str(repo_dir), check=True)
    subprocess.run(["git", "commit", "-m", "fix explicitScrapedRegions to include us region so exact US prices display"], cwd=str(repo_dir), check=True)
...
print("\n🎉 MASTER US REGION PRICE FIX DEPLOYED LIVE!")
