...
class TestBridgeGeoRouting(unittest.TestCase):
    ...
    def test_geo_ui_shipping_badge_and_price_labeling(self):
        ...
        self.assertIn("✨ VERIFIED DEAL", tag)
        self.assertIn("amazon.com", href)
        self.assertIn("smartdeal0358-20", href)
        page.close()

        # TEST 2: ?country=IN (Direct Listing: B0CX144DHK)
        page = context.new_page()
        ...
        self.assertIn("✨ VERIFIED DEAL", tag, "Verified direct regional listing MUST show VERIFIED DEAL tag")
        self.assertIn("amazon.in/dp/B0CX144DHK", href)
        self.assertIn("smartdeal0358-21", href)
        page.close()

        # TEST 3: ?country=IN (Unlisted Search Fallback: B0BZXNSW5K / B0BXP7YWHJ)
        page = context.new_page()
        ...
        self.assertNotIn("✨ VERIFIED DEAL", tag, "Unlisted reseller/search fallback MUST NOT show VERIFIED DEAL tag")
        self.assertIn("⚠️ UNLISTED IN REGION", tag, "Unlisted reseller/search fallback MUST show UNLISTED IN REGION tag")
        self.assertIn("Approx.", price, "Unlisted reseller/search fallback price MUST be labeled as Approx.")
        self.assertIn("amazon.in/s?k=", href)
        self.assertIn("smartdeal0358-21", href)
        page.close()

        # TEST 4: ?country=GB (OneLink UK Unlisted ASIN)
        page = context.new_page()
        ...
        self.assertNotIn("✨ VERIFIED DEAL", tag, "OneLink country without verified regional ASIN MUST NOT show VERIFIED DEAL tag")
        self.assertIn("⚠️ UNLISTED IN REGION", tag)
        ...
        self.assertNotIn("✨ VERIFIED DEAL", tag)
